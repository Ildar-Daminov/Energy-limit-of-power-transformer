function [HST_max,TOT_max,AEQ,HST,TOT]=OF(PUL,AMB)
%% This function calculates thermal mode for OF power transformer per IEC 60076-7
% Input data:
...
%% Constants
% Thermal characteristics of OF power transformer per IEC60076-7
delta_theta_or = 56;     % Top-oil (in tank) temperature rise in steady state at rated losses (no-load losses + load losses),K
delta_theta_hr = 22;     % Hot-spot-to-top-oil (in tank) gradient at rated current, K
...
k21 = 1.3;               % Thermal model constant
...
% % Тепловая модель силового трансформатора OF IEC 60076-7
% % Ввод параметров трансформатора из таблицы в приложении Е
% delta_theta_or = 56;     % input('введите delta_theta_or ');    % Рост температуры в верхних слоях в нормальном режиме, К
% delta_theta_hr = 22;     % input ('введите delta_theta_hr ');   % Превышение температуры ННТ над температурой в верхних слоях масла, К
% tao_0 = 90;             % input ('введите tao_0 ');            % Постоянная времени масла, мин
% tao_w = 7;               % input ('введите tao_w ');           % Постоянная времени обмоток, мин
% R = 6;                   % input ('введите R ');                % постоянная отношение потерь при номинальной нагрузке к потерям на холостом ходу, мин
% x = 1;                 % input ('введите x ');
% y = 1.3;                 % input ('введите y ');
% k11 = 1;               % input ('введите k11 ');
% k21 = 1.3;                 % input ('введите k21 ');
% k22 = 1;                 % input ('введите k22 ');
% %t=5;
% 
